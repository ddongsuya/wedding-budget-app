         ...
         <div className="fixed inset-0 z-[70] bg-black/50 flex items-center justify-center p-4 animate-fade-in" onClick={(e) => { e.stopPropagation(); setIsOpen(false); }} role="dialog" aria-modal="true" aria-label="날짜 선택">
            <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
               ...
               <div className="bg-rose-500 p-5 text-white flex justify-between items-start shrink-0">
                  ...
                  <button type="button" onClick={() => setIsOpen(false)} className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center hover:bg-white/20 rounded-full transition-colors touch-feedback" aria-label="닫기"><X size={24} aria-hidden="true" /></button>
               </div>
                  ...
                  {step === 'day' && (
                     ...
                     <div className="flex-1 flex flex-col animate-slide-up" role="grid" aria-label="달력">
                        <div className="flex justify-between items-center mb-4 px-2">
                           <button type="button" onClick={() => { const d = new Date(viewDate); d.setMonth(d.getMonth() - 1); setViewDate(d); }} className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center hover:bg-stone-100 rounded-full text-stone-600 touch-feedback" aria-label="이전 달"><ChevronLeft size={20} aria-hidden="true" /></button>
                           <button type="button" onClick={() => setStep('month')} className="font-bold text-stone-800 hover:text-rose-500 text-lg min-h-[44px] px-2 flex items-center" aria-label={`${viewDate.getFullYear()}년 ${viewDate.getMonth() + 1}월, 클릭하여 월 선택`}>{viewDate.getFullYear()}년 {viewDate.getMonth() + 1}월</button>
                           <button type="button" onClick={() => { const d = new Date(viewDate); d.setMonth(d.getMonth() + 1); setViewDate(d); }} className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center hover:bg-stone-100 rounded-full text-stone-600 touch-feedback" aria-label="다음 달"><ChevronRight size={20} aria-hidden="true" /></button>
                        </div>
                  ...
                  {step === 'month' && (
                     ...
                     <div className="flex-1 flex flex-col animate-slide-up" role="listbox" aria-label="월 선택">
                       <div className="flex justify-between items-center mb-6 px-2">
                          <button type="button" onClick={() => handleYearSelect(viewDate.getFullYear() - 1)} className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center hover:bg-stone-100 rounded-full text-stone-600 touch-feedback" aria-label="이전 년도"><ChevronLeft size={20} aria-hidden="true" /></button>
                          <button type="button" onClick={() => setStep('year')} className="font-bold text-stone-800 hover:text-rose-500 text-lg mx-auto min-h-[44px] px-2 flex items-center" aria-label={`${viewDate.getFullYear()}년, 클릭하여 년도 선택`}>{viewDate.getFullYear()}년</button>
                          <button type="button" onClick={() => handleYearSelect(viewDate.getFullYear() + 1)} className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center hover:bg-stone-100 rounded-full text-stone-600 touch-feedback" aria-label="다음 년도"><ChevronRight size={20} aria-hidden="true" /></button>
                       </div>
