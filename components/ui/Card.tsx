...
export const Card: React.FC<CardProps> = memo(({ children, className = '', title, action, interactive = false }) => {
  return (
    <div 
      className={`bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden transition-all duration-200 ${interactive ? 'card-interactive hover:shadow-soft-lg active:scale-[0.99]' : ''} ${className}`}
...
export const SummaryCard: React.FC<SummaryCardProps> = memo(({ label, value, icon, trend, delay = 0 }) => (
  ...
  >
    <div className="flex justify-between items-start z-10">
      ...
      {icon && <div className="text-rose-500 p-2 bg-rose-50 rounded-lg group-hover:scale-110 transition-transform duration-300" aria-hidden="true">{icon}</div>}
    </div>
    <div className="z-10">
    ...
    {/* Decorative circle */}
    <div className="absolute -bottom-4 -right-4 w-20 h-20 bg-gradient-to-br from-rose-50 to-transparent rounded-full opacity-50 group-hover:scale-125 transition-transform duration-500" aria-hidden="true"></div>
  </div>
));
